...
{
    if(!(UART0->FR & UART_FR_RXFE))//Check if data rx flag is set, flag checks if data has received
    {
        __delay_cycles(16000);//NOP 1ms
        for(index_buffer_RX = 0; index_buffer_RX <= buffer_size-1; index_buffer_RX++)//clear complete buffer
        {buffer_RX[index_buffer_RX] = 0x00;}//set default value for char array

        index_buffer_RX = 0;//reset Index counter
        do//do read data as long buffer is not empty and buffer size has not reached his end
        {
        ...
        UARTSendArray("You Send:  ");//send back "You Send" as a loop back
        UARTSendArray(buffer_RX);//send back data package
        UARTSendArray("\r\n");//send newline
        //external function
        extern CommandDecoder(char input_command[]);
